...
class ActiveTimecodeApp:
    def __init__(self, master):
        ...
        self.current_mode = tk.StringVar(value=self.settings.get("current_mode", "방송 진행 모드")) # 초기 모드 설정
        ...
        mode_menu.add_radiobutton(label="방송 진행 모드", variable=self.current_mode, value="방송 진행 모드", command=self.set_mode)
        mode_menu.add_radiobutton(label="GTO-W 감시 모드", variable=self.current_mode, value="GTO-W 감시 모드", command=self.set_mode)
        # --- 모드 선택 메뉴 추가 끝 ---
    ...
    def update_timecode(self):
        ...
        if self.current_mode.get() == "GTO-W 감시 모드":
            self.check_gto_rules()

    def check_gto_rules(self):
        b_column_values = [] # 이 부분을 루프 밖으로 이동
        # 모든 B 열 엔트리 위젯의 배경색 초기화
        for i, line_entry_data in enumerate(self.line_entries):
            b_value_str = line_entry_data["button"].get().strip()
            try:
                val = int(b_value_str) if b_value_str else 0
                b_column_values.append(val)
                line_entry_data["button"].config(bg="#333") # 기본 배경색
            except ValueError:
                b_column_values.append(-1) # 유효하지 않은 값은 -1로 처리
                line_entry_data["button"].config(bg="orange") # 유효하지 않은 값은 주황색으로 표시
                self.push_error(f"레일 {i+1}의 'B' 열에 유효하지 않은 값('{b_value_str}')이 입력되었습니다. 숫자를 입력해주세요.")
        ...
        for i, val in enumerate(b_column_values):
            if val == 2: # 계획의 시작
                if current_plan: # 이전 계획이 끝나지 않았다면 저장
                    gto_plans.append((current_plan, i - len(current_plan))) # (계획, 시작 인덱스)
                current_plan = [(val, i)] # (값, 인덱스)
            elif val == 1: # 계획의 끝
                current_plan.append((val, i))
                gto_plans.append((current_plan, i - len(current_plan) + 1)) # (계획, 시작 인덱스)
                current_plan = []
            elif current_plan: # 계획 진행 중
                current_plan.append((val, i))
        if current_plan: # 마지막 계획이 끝나지 않았다면 저장
            gto_plans.append((current_plan, len(b_column_values) - len(current_plan)))

        # 각 GTO 계획에 대해 규칙 검사
        for plan_data, start_idx in gto_plans:
            plan_values = [item[0] for item in plan_data]
            plan_indices = [item[1] for item in plan_data]
            
            is_valid_plan = True
            error_messages = []

            # 1. 모든 숫자는 두 번 이상 연속해서 작성될 수 없다.
            for i in range(len(plan_values) - 1):
                if plan_values[i] == plan_values[i+1] and plan_values[i] != 0: # 0은 연속 허용
                    error_messages.append(f"규칙 1 위반: 레일 {plan_indices[i]+1}, {plan_indices[i+1]+1} - '{plan_values[i]}'가 연속으로 작성되었습니다.")
                    is_valid_plan = False
                    break
            if not is_valid_plan: 
                for _, original_idx in plan_data:
                    self.line_entries[original_idx]["button"].config(bg="#FFCCCC") # 규칙 위반 시 배경색 변경
                continue # 다음 계획으로
        ...
        for i, val in enumerate(b_column_values):
            if val == 2: # 계획의 시작
                ...
                current_plan.append((val, i))
                if len(current_plan) >= 3: # 최소 3줄 이상인 계획만 유효
                    gto_plans.append((current_plan, i - len(current_plan) + 1)) # (계획, 시작 인덱스)
                else:
                    self.push_error(f"레일 {current_plan[0][1]+1}부터 시작하는 GTO 계획이 너무 짧습니다. 최소 3줄 이상이어야 합니다.")
                current_plan = []
            elif current_plan: # 계획 진행 중
                current_plan.append((val, i))
        if current_plan: # 마지막 계획이 끝나지 않았다면 저장
        ...
        for plan_data, start_idx in gto_plans:
            ...
            # 2. GTO 계획의 처음은 반드시 2 - 4 - 5 이다.
            # 3. GTO 계획의 마지막은 반드시 다음 둘 중의 하나 이다. (5 - 8 - 1 또는 6 - 17 - 1)
            # 4. 위의 2번과 3번 규칙을 지키지 않아도 되는 숫자 열은 다음의 단 하나 이다. (2 - 4 - 17 - 1)
            
            # 특별 예외 처리: 2 - 4 - 17 - 1
            if plan_values == [2, 4, 17, 1]:
                # 이 경우는 규칙 2, 3을 무시하므로 다음 규칙으로 넘어감
                pass
            else:
                # 규칙 2 검사
                if len(plan_values) >= 3 and not (plan_values[0] == 2 and plan_values[1] == 4 and plan_values[2] == 5):
                    error_messages.append(f"규칙 2 위반: 레일 {plan_indices[0]+1} - GTO 계획의 시작은 반드시 '2 - 4 - 5'여야 합니다. 현재: {plan_values[0]}, {plan_values[1]}, {plan_values[2]}")
                    is_valid_plan = False
                
                # 규칙 3 검사
                if len(plan_values) >= 3 and not ((plan_values[-3:] == [5, 8, 1]) or (plan_values[-3:] == [6, 17, 1])):
                    error_messages.append(f"규칙 3 위반: 레일 {plan_indices[-3]+1} - GTO 계획의 마지막은 반드시 '5 - 8 - 1' 또는 '6 - 17 - 1'이어야 합니다. 현재: {plan_values[-3]}, {plan_values[-2]}, {plan_values[-1]}")
                    is_valid_plan = False
            ...
            # 5. 2는 반드시 GTO 계획의 시작에만 작성되어야 한다. (2와 1은 반드시 교차해서 등장해야 한다.)
            for i, val in enumerate(plan_values):
                if val == 2 and i != 0:
                    error_messages.append(f"규칙 5 위반: 레일 {plan_indices[i]+1} - '2'는 GTO 계획의 시작에만 작성되어야 합니다.")
                    is_valid_plan = False
                    break
                if val == 2 and i > 0 and plan_values[i-1] == 2: # 연속 2 방지 (규칙 1에서 이미 처리되지만 명시적으로)
                    error_messages.append(f"규칙 5 위반: 레일 {plan_indices[i]+1} - '2'는 연속해서 작성될 수 없습니다.")
                    is_valid_plan = False
                    break
                if val == 1 and i > 0 and plan_values[i-1] == 1: # 연속 1 방지 (규칙 1에서 이미 처리되지만 명시적으로)
                    error_messages.append(f"규칙 5 위반: 레일 {plan_indices[i]+1} - '1'은 연속해서 작성될 수 없습니다.")
                    is_valid_plan = False
                    break
            
            # 2와 1의 교차 등장 (시작과 끝이 명확한 계획 내에서)
            if plan_values[0] == 2 and plan_values[-1] == 1:
                two_count = plan_values.count(2)
                one_count = plan_values.count(1)
                if two_count != 1 or one_count != 1: # 시작과 끝의 2, 1은 하나씩만 있어야 함
                    error_messages.append(f"규칙 5 위반: 레일 {plan_indices[0]+1} - '2'와 '1'은 계획 내에서 한 번씩만 등장해야 합니다.")
                    is_valid_plan = False
                
                # 2와 1이 교차하는지 확인 (간단화된 검사)
                if len(plan_values) > 2:
                    first_two_idx = -1
                    last_one_idx = -1
                    for i, val in enumerate(plan_values):
                        if val == 2: first_two_idx = i
                        if val == 1: last_one_idx = i
                    if first_two_idx >= last_one_idx:
                        error_messages.append(f"규칙 5 위반: 레일 {plan_indices[0]+1} - '2'와 '1'의 순서가 올바르지 않습니다.")
                        is_valid_plan = False
            ...
            # 6. 4는 GTO 계획의 두 번째 레일을 제외하면 그 어디에도 작성될 수 없다.
            for i, val in enumerate(plan_values):
                if val == 4 and i != 1:
                    error_messages.append(f"규칙 6 위반: 레일 {plan_indices[i]+1} - '4'는 GTO 계획의 두 번째 레일에만 작성되어야 합니다.")
                    is_valid_plan = False
                    break
            if not is_valid_plan: 
                for _, original_idx in plan_data:
            ...
            # 7. 5는 반드시 4 혹은 6 다음에만 작성되어야 한다.
            for i, val in enumerate(plan_values):
                if val == 5 and i > 0 and plan_values[i-1] not in [4, 6]:
                    error_messages.append(f"규칙 7 위반: 레일 {plan_indices[i]+1} - '5'는 '4' 또는 '6' 다음에만 작성되어야 합니다.")
                    is_valid_plan = False
                    break
            if not is_valid_plan: 
                for _, original_idx in plan_data:
            ...
            # 8. 6은 반드시 5 혹은 7 다음에만 작성되어야 한다.
            for i, val in enumerate(plan_values):
                if val == 6 and i > 0 and plan_values[i-1] not in [5, 7]:
                    error_messages.append(f"규칙 8 위반: 레일 {plan_indices[i]+1} - '6'은 '5' 또는 '7' 다음에만 작성되어야 합니다.")
                    is_valid_plan = False
                    break
            if not is_valid_plan: 
                for _, original_idx in plan_data:
            ...
            # 9. 7은 반드시 5 다음에만 작성되어야 하고, 7 다음은 반드시 6 이 작성되어야 한다.
            for i, val in enumerate(plan_values):
                if val == 7:
                    if i == 0 or plan_values[i-1] != 5:
                        error_messages.append(f"규칙 9 위반: 레일 {plan_indices[i]+1} - '7'은 '5' 다음에만 작성되어야 합니다.")
                        is_valid_plan = False
                        break
                    if i == len(plan_values) - 1 or plan_values[i+1] != 6:
                        error_messages.append(f"규칙 9 위반: 레일 {plan_indices[i]+1} - '7' 다음에는 반드시 '6'이 작성되어야 합니다.")
                        is_valid_plan = False
                        break
            if not is_valid_plan: 
                for _, original_idx in plan_data:
            ...
            # 10. 8과 17은 GTO 계획의 마지막에서 두 번째 레일을 제외하면 그 어디에도 작성될 수 없다.
            for i, val in enumerate(plan_values):
                if val in [8, 17] and i != len(plan_values) - 2:
                    error_messages.append(f"규칙 10 위반: 레일 {plan_indices[i]+1} - '8' 또는 '17'은 GTO 계획의 마지막에서 두 번째 레일에만 작성되어야 합니다.")
                    is_valid_plan = False
                    break
            if not is_valid_plan: 
                for _, original_idx in plan_data:
            ...
            # 최종 결과 처리
            if is_valid_plan:
                # 12. 감시를 통과한 GTO 계획은 자동으로 모든 레일의 <Run> 버튼이 켜져야 하고, 이것을 실행된 것으로 본다.
                # 이제 check_gto_rules는 실행 상태를 직접 변경하지 않고, 유효성만 표시합니다.
                for _, original_idx in plan_data:
                    self.line_entries[original_idx]["button"].config(bg="#CCFFCC") # 유효한 계획은 연한 녹색으로 표시
                self.push_error(f"레일 {plan_indices[0]+1}부터 시작하는 GTO 계획이 유효합니다.")
            else:
                # 13. 감시를 통과하지 못한 GTO 계획은 실행되어서는 안되며, 사용자에게 알람 또는 수정할 수 있는 기회를 제공해야 한다.
                for msg in error_messages:
                    self.push_error(msg)
                self.push_error(f"레일 {plan_indices[0]+1}부터 시작하는 GTO 계획이 규칙을 위반했습니다. 실행되지 않습니다.")
                # 실행 중인 경우 중지
                for _, original_idx in plan_data:
                    self.stop_line(original_idx) # stop_line은 monitor_status를 업데이트함
            ...
            if is_valid_plan:
                # 12. 감시를 통과한 GTO 계획은 자동으로 모든 레일의 <Run> 버튼이 켜져야 하고, 이것을 실행된 것으로 본다.
                for _, original_idx in plan_data:
                    self.run_line(original_idx) # run_line은 monitor_status를 업데이트함
                self.push_error(f"레일 {plan_indices[0]+1}부터 시작하는 GTO 계획이 감시를 통과했습니다. 자동 실행됩니다.")
            else:
                # 13. 감시를 통과하지 못한 GTO 계획은 실행되어서는 안되며, 사용자에게 알람 또는 수정할 수 있는 기회를 제공해야 한다.
                for msg in error_messages:
                    self.push_error(msg)
    ...
    def set_mode(self):
        ...
        self.push_error(f"모드가 '{selected_mode}'(으)로 변경되었습니다.")
        # 여기에 모드 변경에 따른 추가 로직을 구현할 수 있습니다.
        # 예: GTO-W 감시 모드일 때만 특정 UI 요소 활성화/비활성화
